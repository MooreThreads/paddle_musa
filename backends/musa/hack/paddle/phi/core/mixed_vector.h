...
inline paddle::optional<phi::CustomPlace> OptionalCUDAPlace(
...
class MixVector {
  ...
  class PADDLE_API VectorData {
   public:
    ...
    const T *CUDAData(phi::Place place) const {
      PADDLE_ENFORCE_EQ(
          place.GetType() == phi::AllocationType::GPU || place.GetType() == phi::AllocationType::CUSTOM,
          true,
    ...
    paddle::optional<phi::CustomPlace> CUDAPlace() const {
 ...
 public:
  ...
  const T *CUDAData(phi::Place place) const {
    {
      phi::CustomPlace p("musa", place.GetDeviceId());
      auto &mtx = m_->Mutex();
