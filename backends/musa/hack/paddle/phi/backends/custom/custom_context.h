...
class CustomContext : public DeviceContext,
 ...
 public:
  ...
  mudnnHandle_t cudnn_handle() const;

  mublasHandle_t cublas_handle() const;

  mublasLtHandle_t cublaslt_handle() const;

  void SetBlasHandle(mublasHandle_t);
  void SetBlasHandle(std::function<mublasHandle_t()>&&);

  void SetBlasTensorCoreHandle(mublasHandle_t);
  void SetBlasTensorCoreHandle(std::function<mublasHandle_t()>&&);

  void SetBlasTF32Handle(mublasHandle_t);
  void SetBlasTF32Handle(std::function<mublasHandle_t()>&&);

  void SetBlasLtHandle(mublasLtHandle_t);
  void SetBlasLtHandle(std::function<mublasLtHandle_t()>&&);

  bool tensor_core_available() const;

  void CublasCall(const std::function<void(mublasHandle_t)>&) const;
  void CublasCall(const std::function<void(int)>&) const;

  void TensorCoreCublasCallIfAvailable(
      const std::function<void(mublasHandle_t)>&) const;
