...
#define PADDLE_WITH_CUDA PADDLE_WITH_MUSA
#define PADDLE_CUDA_BF16 PADDLE_MUSA_BF16
...
/////////////////////////////////////////////////////////////////////
static __device__ __forceinline__ uint32_t lop3_c_func(uint32_t a, uint32_t b, uint32_t c, uint8_t immLut) {
  uint32_t out = 0;
  #pragma unroll
  for (int bit = 0; bit < 32; ++bit) {
      uint32_t abit = (a >> bit) & 1;
      uint32_t bbit = (b >> bit) & 1;
      uint32_t cbit = (c >> bit) & 1;

      uint32_t idx = (abit << 2) | (bbit << 1) | cbit;
      uint32_t outbit = (immLut >> idx) & 1;

      out |= (outbit << bit);
  }
  return out;
}
static __device__ __forceinline__ uint32_t fma_bf16x2_c_func(uint32_t a, uint32_t b, uint32_t c) {
  __half2 r = __hfma2(*reinterpret_cast<__half2*>(&a), 
                      *reinterpret_cast<const __half2*>(&b), 
                      *reinterpret_cast<const __half2*>(&c));
  return *reinterpret_cast<uint32_t*>(&r); 
}
static __device__ __forceinline__ uint32_t sub_f16x2_c_func(uint32_t a, uint32_t b) {
  __half2 r = __hsub2(*reinterpret_cast<__half2*>(&a), *reinterpret_cast<const __half2*>(&b));
  return *reinterpret_cast<uint32_t*>(&r);
}
...
template <>
struct CUDA_HALF_2_TYPE_TARIS<__mt_bfloat16> {
...
__device__ inline void fast_cvt_4_packed_signed_i8s_to_2_half2s(
  ...
  h[0] = sub_f16x2_c_func(h[0], I8s_TO_F16s_MAGIC_NUM);
  // asm volatile("sub.f16x2 %0, %1, %2;\n"
  //              : "=r"(h[1])
  //              : "r"(h[1]), "r"(I8s_TO_F16s_MAGIC_NUM));
  h[1] = sub_f16x2_c_func(h[1], I8s_TO_F16s_MAGIC_NUM);
#endif
}
...
struct WeightLayoutDetails<WeightOnlyQuantType::Int4b> {
  ...
  template <const int Num, const int WarpSize>
...
struct WeightOnlyConverter<half, WeightOnlyQuantType::Int4b> {
  static __device__ inline void convert(half halves[8],
    ...
    h[0] = lop3_c_func(i4s, BOTTOM_MASK, I4s_TO_F16s_MAGIC_NUM, immLut);
    // Extract elt_23 (i4s & 0x00f000f0) | 0x64006400
    // asm volatile(
    //     "lop3.b32 %0, %1, %2, %3, %4;\n"
    //     : "=r"(h[1])
    //     : "r"(i4s), "n"(TOP_MASK), "n"(I4s_TO_F16s_MAGIC_NUM), "n"(immLut));
    h[1] = lop3_c_func(i4s, TOP_MASK, I4s_TO_F16s_MAGIC_NUM, immLut);
    ...
    //                "n"(immLut));
    h[2] = lop3_c_func(top_i4s, BOTTOM_MASK, I4s_TO_F16s_MAGIC_NUM, immLut);
    // Extract elt_67 (top_i4s & 0x00f000f0) | 0x64006400
    // asm volatile(
    //     "lop3.b32 %0, %1, %2, %3, %4;\n"
    //     : "=r"(h[3])
    //     : "r"(top_i4s), "n"(TOP_MASK), "n"(I4s_TO_F16s_MAGIC_NUM), "n"(immLut));
    h[3] = lop3_c_func(top_i4s, TOP_MASK, I4s_TO_F16s_MAGIC_NUM, immLut);
    ...
    h[0] = sub_f16x2_c_func(h[0], FP16_TOP_MAGIC_NUM);
    // Convert elt_23
    // asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n"
    //              : "=r"(h[1])
    //              : "r"(h[1]), "r"(ONE_SIXTEENTH), "r"(NEG_72));
    h[1] = fma_bf16x2_c_func(h[1], ONE_SIXTEENTH, NEG_72);
    // Convert elt_45
    // asm volatile("sub.f16x2 %0, %1, %2;\n"
    //              : "=r"(h[2])
    //              : "r"(h[2]), "r"(FP16_TOP_MAGIC_NUM));
    h[2] = sub_f16x2_c_func(h[2], FP16_TOP_MAGIC_NUM);
    // Convert elt_67
    // asm volatile("fma.rn.f16x2 %0, %1, %2, %3;\n"
    //              : "=r"(h[3])
    //              : "r"(h[3]), "r"(ONE_SIXTEENTH), "r"(NEG_72));
    h[3] = fma_bf16x2_c_func(h[1], ONE_SIXTEENTH, NEG_72);
  }
};
...
struct WeightOnlyConverter<__nv_bfloat16, WeightOnlyQuantType::Int4b> {
  static __device__ inline void convert(__nv_bfloat16 halves[8],
    ...
    h[0] = lop3_c_func(i4s, MASK, I4s_TO_BF16s_MAGIC_NUM, immLut);
#pragma unroll
    for (int ii = 1; ii < 4; ++ii) {
      ...
      //     : "=r"(h[ii])
      //     : "r"(i4s), "n"(MASK), "n"(I4s_TO_BF16s_MAGIC_NUM), "n"(immLut));
      h[ii] = lop3_c_func(i4s, MASK, I4s_TO_BF16s_MAGIC_NUM, immLut);
    }
...
#pragma unroll
    for (int ii = 0; ii < 4; ++ii) {
      ...
      h[ii] = fma_bf16x2_c_func(h[ii], BF16_ONE, BF16_BIAS);
    }
