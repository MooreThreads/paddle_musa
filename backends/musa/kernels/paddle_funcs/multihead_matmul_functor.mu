...
__global__ void SoftmaxKernelWithEltadd<half>(
    ...
    const int seq_len,
    const phi::funcs::warp_mask_t mask) {
#if (defined(PADDLE_WITH_CUDA) && CUDA_ARCH_FP16_SUPPORTED(__CUDA_ARCH__)) || defined(PADDLE_WITH_MUSA)
  int qk_offset = blockIdx.x * seq_len;
  assert(blockDim.x % WARP_SIZE == 0);
...
inline void MatmulWithHeadQK(const GPUContext &dev_ctx,
  ...
  auto blas = ::phi::funcs::GetBlas<GPUContext, run_type>(dev_ctx);
  ...
  if (seq_len <= 1024) {
    ...
    if (seq_len % 2 == 0) {
      ...
      if (std::is_same<T, float>::value) {
        ...
        if (bias_is_mask) {
#if defined(__HIPCC__) || (defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 700) || defined(__MUSACC__)
