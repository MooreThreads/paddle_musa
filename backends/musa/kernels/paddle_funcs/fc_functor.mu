...
__global__ void InplaceAddReluKernel(const int N, const T* bias, T* data) {
  ...
  for (int i = threadIdx.x; i < N; i += BlockDim) {
    T temp;
#if defined(__HIPCC__) || __CUDA_ARCH__ >= 350 || defined(__MUSACC__)
...
void LaunchBiasAddReluHalf2Kernel(musaStream_t stream,
                                  const int32_t rows,
                                  const int32_t cols,
                                  float16* Y,
                                  const float16* B,
 ...
 **/
void DispatchBiasAddReluKernelHalf2VecSize(musaStream_t stream,
                                           const int32_t rows,
                                           const int32_t cols,
                                           float16* Y,
                                           const float16* B,
...
template <>
void AddReluKernel(musaStream_t stream,
                   const int M,
                   const int N,
                   float16* Y,
                   const float16* B,
...
#else
  musaMemcpyAsync(scale_weights_dev_ptr,
                  scale_weights.data(),
                  N * sizeof(float),
