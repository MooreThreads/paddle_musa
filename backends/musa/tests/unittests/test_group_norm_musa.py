...
class TestGroupNormAPIV2_With_NCHW(unittest.TestCase):
    def test_numerical_accuracy(self):
        paddle.disable_static()
        shape = (2, 6, 2, 4)
        np.random.seed(10)
        for place in get_places():
            ...
            data = np.random.random(shape).astype("float32")
            expect_res1 = group_norm_naive_for_general_dimension(
                data, scale, bias, epsilon=1e-5, groups=6, channel_last=False
            )
            expect_res2 = group_norm_naive_for_general_dimension(
                data, scale, bias, epsilon=1e-5, groups=2, channel_last=False
            )

            gn1 = paddle.nn.GroupNorm(
                num_channels=6, num_groups=6, data_format='NCHW'
            )
            gn2 = paddle.nn.GroupNorm(
                num_channels=6, num_groups=2, data_format='NCHW'
            )
            data_pd = paddle.to_tensor(data)
            result1 = gn1(data_pd).numpy()
            result2 = gn2(data_pd).numpy()
...
class TestGroupNormAPIV2_With_NDHWC(unittest.TestCase):
    def test_numerical_accuracy(self):
        ...
        for place in get_places():
            ...
            data_pd = paddle.to_tensor(data)
            print(f"TEST..............data_pd={data_pd}")
            result1 = gn1(data_pd)
            result2 = gn2(data_pd)
            print(f"TEST..............gn1={result1}")
            print(f"TEST..............gn2={result2}")
            #np.testing.assert_allclose(result1, expect_res1, atol=1e-5)
            #np.testing.assert_allclose(result2, expect_res2, atol=1e-5)
