...
# TODO: opt clear_grad() needs more op implementation
class TestAdamOp(unittest.TestCase):
    def test_adam_op_forward_all_dtypes(self):
        shape = (3, 4, 5)
        for dtype in ("float32", "float64"):
            with self.subTest(dtype=dtype):
                ...
                optimizer = paddle.optimizer.Adam(learning_rate=0.1, parameters=[x])
                y = x**2
                y.backward()
